...
export default function EditorArea({ text, setText, user }: Props) {
  const textareaRef = useRef<HTMLTextAreaElement>(null);
  const [isFlashing, setIsFlashing] = useState(false);

  // --- 核心修改：动态判定限制 ---
  const maxLimit = user ? 1500 : 300;
        ...
        <div className="mt-6 w-full max-w-lg mx-auto">
          <div className="flex justify-between items-end mb-2 px-1">
            <div className="flex items-center gap-2">
              ...
              {!user && (
                <span className="text-[9px] font-bold text-[#9C27B0] animate-pulse bg-[#9C27B0]/5 px-2 py-0.5 rounded">
                  登录解锁 1500 字
                </span>
              )}
            </div>
            <span className={`text-[11px] font-black ${pureTextLength > maxLimit ? 'text-rose-500' : 'text-gray-400'}`}>
              {pureTextLength} / {maxLimit} 字
            </span>
          </div>
          <div className="w-full h-[6px] bg-[#F5F5F7] rounded-full overflow-hidden shadow-inner">
            <div 
              className={`h-full transition-all duration-500 ${pureTextLength > maxLimit ? 'bg-rose-500' : 'bg-[#9C27B0]'}`} 
              style={{ width: `${Math.min((pureTextLength / maxLimit) * 100, 100)}%` }} 
            />
          </div>
        </div>
      </div>
